    ...
    m_pTooltipItem(NULL)
{
    ...
    //}}AFX_DATA_INIT
    std::string folderPath = "DataFiles\\UIImages\\";
    LoadImageFile(folderPath, m_tree.Background(), &m_imageBackground, CSize(c_sizeX, c_sizeY));
...
BOOL CDestinyTreeDialog::OnEraseBkgnd(CDC*)
{
    return 0;
}
...
void CDestinyTreeDialog::OnPaint()
{
    ...
    if (m_tree.Items().size() > 0)
    {
        ...
        CImage image;
        LoadImageFile("DataFiles\\UIImages\\", m_tree.Icon(), &image, CSize(32, 32));
        ...
        size_t spentInTree = m_pCharacter->ActiveBuild()->APSpentInTree(m_tree.Name());
        size_t available = m_pCharacter->ActiveBuild()->TotalPointsAvailable(0, m_tree.Name(), m_type);
        CString text;
        ...
        CDC * pDC)
{
    ...
    else
    {
        ...
        bool isAllowed = item.IsAllowed(*m_pCharacter->ActiveBuild(), "", m_tree.Name(), 0);
        if (isAllowed)
        {
        ...
        CDC * pDC)
{
    ...
    size_t spentInTree = m_pCharacter->ActiveBuild()->APSpentInTree(m_tree.Name());
    bool isAllowed = item.IsAllowed(*m_pCharacter->ActiveBuild(), "", m_tree.Name(), 0);
    bool isTrainable = item.CanTrain(*m_pCharacter->ActiveBuild(), m_tree.Name(), spentInTree, m_type, 0);
    // now apply the item position to the rectangle
        ...
        const CRect & itemRect)
{
    ...
    {
        item.RenderIcon(*m_pCharacter->ActiveBuild(), pDC, itemRect);
    }
}
...
void CDestinyTreeDialog::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (m_tree.Items().size() > 0)
    {
        ...
        if (item != NULL)
        {
            ...
            size_t spentInTree = m_pCharacter->ActiveBuild()->APSpentInTree(m_tree.Name());
            const TrainedEnhancement * te = m_pCharacter->ActiveBuild()->IsTrained(item->InternalName(), "");
            bool isAllowed = item->MeetRequirements(*m_pCharacter->ActiveBuild(), "", m_tree.Name(), 0);
            bool isTrainable = item->CanTrain(*m_pCharacter->ActiveBuild(), m_tree.Name(), spentInTree, m_type, 0);
        ...
        if (rctResetButton.PtInRect(point))
        {
            ...
            if (spentInTree > 0)
            {
                ...
                if (ret == IDYES)
                {
                    ...
                    m_pCharacter->ActiveBuild()->EpicDestiny_ResetEnhancementTree(m_tree.Name(), false);
                }
            }
        }
    }
...
void CDestinyTreeDialog::OnRButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (m_pCharacter != NULL)
    {
        ...
        {
            ASSERT(m_type == TT_epicDestiny);
            m_pCharacter->ActiveBuild()->EpicDestiny_RevokeEnhancement(m_tree.Name());
            Invalidate();   // redraw
        }
    }
}
...
void CDestinyTreeDialog::OnMouseMove(UINT, CPoint)
{
...
LRESULT CDestinyTreeDialog::OnMouseLeave(WPARAM, LPARAM)
{
    ...
    else
    {
        ...
        m_tooltip.SetEnhancementTreeItem(
                *m_pCharacter->ActiveBuild(),
                &item,
                selection,
                m_pCharacter->ActiveBuild()->APSpentInTree(m_tree.Name()));
    }
    m_tooltip.Show();
}
...
void CDestinyTreeDialog::OnMButtonDown(UINT, CPoint)
{
