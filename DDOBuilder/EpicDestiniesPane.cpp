// EpicDestiniesPane.cpp
//
#include "stdafx.h"
#include "EpicDestiniesPane.h"
...
void CEpicDestiniesPane::DoDataExchange(CDataExchange* pDX)
{
...
#ifdef _DEBUG
void CEpicDestiniesPane::AssertValid() const
{
    CFormView::AssertValid();
}

#ifndef _WIN32_WCE
void CEpicDestiniesPane::Dump(CDumpContext& dc) const
{
    CFormView::Dump(dc);
}
#endif
...
LRESULT CEpicDestiniesPane::OnLoadComplete(WPARAM, LPARAM)
{
    return 0;
}

// Character Observer overrides
void CEpicDestiniesPane::UpdateActiveLifeChanged(Character*)
{
...
void CEpicDestiniesPane::UpdateActiveBuildChanged(Character*)
{
    Life *pLife = m_pCharacter->ActiveLife();
    Build *pBuild = m_pCharacter->ActiveBuild();
    if (pLife != NULL
...
void CEpicDestiniesPane::OnInitialUpdate()
{
...
void CEpicDestiniesPane::UpdateMouseHooks()
{
    CRect rect;
    GetWindowRect(&rect);
    int cx = rect.Width();
...
CSize CEpicDestiniesPane::MoveFatePointControls()
{
    CSize bottomRight;
    ...
    {
        if (m_pCharacter->ActiveBuild()->HasEpicCompletionist())
        {
            twistIndex = 4;
        }
    }
...
LRESULT CEpicDestiniesPane::OnNewDocument(WPARAM wParam, LPARAM lParam)
{
    if (m_pCharacter != NULL)
    {
...
BOOL CEpicDestiniesPane::OnEraseBkgnd(CDC* pDC)
{
...
std::vector<EnhancementTree> CEpicDestiniesPane::DetermineTrees()
{
...
void CEpicDestiniesPane::CreateEnhancementWindows()
{
    LockWindowUpdate();
...
void CEpicDestiniesPane::DestroyEnhancementWindows()
{
    LockWindowUpdate();
    for (size_t i = 0; i < m_treeViews.size(); ++i)
    {
...
void CEpicDestiniesPane::PopulateCombobox()
{
    ...
    {
        activeDestiny = m_pCharacter->ActiveBuild()->ActiveEpicDestiny();
    }
...
void CEpicDestiniesPane::EnableControls()
{
    if (m_pCharacter != NULL
            && m_pCharacter->ActiveBuild() != NULL)
    {
        ...
        m_buttonMakeActive.SetCheck((m_pCharacter->ActiveBuild()->ActiveEpicDestiny() == destinyName)
                ? BST_CHECKED
                : BST_UNCHECKED);
        ...
        for (size_t vi = 0; vi < m_twistsOfFate.size(); ++vi)
        {
            if (vi == MAX_TWISTS - 1)
            {
                ...
                m_twistsOfFate[vi]->ShowWindow(
                        m_pCharacter->ActiveBuild()->HasEpicCompletionist()
                                ? SW_SHOW
                                : SW_HIDE);
            }
            else
            ...
            m_twistsOfFate[vi]->EnableWindow(m_pCharacter->ActiveBuild()->IsTwistActive(vi));
        }
        // show how many fate points are available
        int totalFatePoints = m_pCharacter->ActiveBuild()->FatePoints();
        size_t spentFatePoints = m_pCharacter->ActiveBuild()->SpentFatePoints();
        CString text;
...
void CEpicDestiniesPane::OnDestinySelect()
{
    int sel = m_comboEpicDestinySelect.GetCurSel();
    if (sel != CB_ERR)
    {
        ...
        {
            // only show the selected tree
            m_treeViews[i]->ShowWindow(i == static_cast<size_t>(sel) ? SW_SHOW : SW_HIDE);
        }
        EnableControls();
    }
}

void CEpicDestiniesPane::OnButtonMakeActiveDestiny()
{
    ...
    if (checked)
    {
        ...
        if (sel != CB_ERR)
        {
            sel = m_comboEpicDestinySelect.GetItemData(sel);
            ASSERT(sel < (int)m_availableEpicDestinies.size());
            m_pCharacter->ActiveBuild()->EpicDestiny_SetActiveDestiny(m_availableEpicDestinies[sel].Name());
        }
    }
    else
    {
        // clear the active destiny
        m_pCharacter->ActiveBuild()->EpicDestiny_SetActiveDestiny("");
    }
    EnableControls();
}

void CEpicDestiniesPane::OnButtonClaimDestinies()
{
        ...
        if ((*it).HasIsEpicDestiny())
        {
            ...
            while (tii != items.end())
            {
                if ((*tii).YPosition() == 0)
                {
                    ...
                    if (!m_pCharacter->ActiveBuild()->IsEnhancementTrained((*tii).InternalName(), "", TT_epicDestiny))
                    {
                        m_pCharacter->ActiveBuild()->EpicDestiny_TrainEnhancement(
                                (*it).Name(),
                                (*tii).InternalName(),
                                "",
...
//void CEpicDestiniesPane::UpdateEnhancementTrained(
//        Character * charData,
...
//}
//
//void CEpicDestiniesPane::UpdateEnhancementRevoked(
//        Character * charData,
//        const std::string & enhancementName,
//        const std::string & selection,
//        bool isTier5)
...
//}
//
//void CEpicDestiniesPane::UpdateEnhancementTreeReset(
//        Character * charData)
//{
//    EnableControls();
//}

void CEpicDestiniesPane::UpdateFatePointsChanged(Build*)
{
    EnableControls();
}

//void CEpicDestiniesPane::UpdateEpicCompletionistChanged(Build*)
//{
//    EnableControls();
//    MoveFatePointControls();
//}
...
LRESULT CEpicDestiniesPane::OnMouseEnter(WPARAM wParam, LPARAM)
{
    if (m_pCharacter != NULL)
    {
        ...
        for (size_t i = 0; i < MAX_TWISTS; ++i)
        {
            ...
            {
                const TrainedEnhancement * te = m_pCharacter->ActiveBuild()->TrainedTwist(i);
                if (te != NULL)
                {
...
LRESULT CEpicDestiniesPane::OnMouseLeave(WPARAM, LPARAM)
{
...
void CEpicDestiniesPane::ShowTip(const EnhancementTreeItem & item, CRect itemRect)
{
    if (m_showingTip)
    {
        m_tooltip.Hide();
...
void CEpicDestiniesPane::HideTip()
{
...
void CEpicDestiniesPane::SetTooltipText(
    ...
    else
    {
        ...
        m_tooltip.SetEnhancementTreeItem(
                *m_pCharacter->ActiveBuild(),
                &item,
                selection,
                m_pCharacter->ActiveBuild()->APSpentInTree(treeName));
    }
    m_tooltip.Show();
}

void CEpicDestiniesPane::OnHScroll(UINT p1, UINT p2, CScrollBar* p3)
{
    // call base class
...
void CEpicDestiniesPane::OnVScroll(UINT p1, UINT p2, CScrollBar* p3)
{
    // call base class
...
void CEpicDestiniesPane::UpdateActionPointsChanged(Build*)
{
    for (size_t i = 0; i < m_treeViews.size(); ++i)
    {
